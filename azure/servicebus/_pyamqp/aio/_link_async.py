...
class Link:  # pylint: disable=too-many-instance-attributes
    ...
    async def _incoming_attach(self, frame) -> None:
        ...
        if not frame[5] and not frame[6]:
            _LOGGER.info("Cannot get source or target. Detaching link", extra=self.network_trace_params)
            await self._set_state(LinkState.DETACHED)
            raise ValueError("Invalid link")
        self.remote_handle = frame[1]  # handle
        self.remote_max_message_size = frame[10] if len(frame) > 10 else 1024 * 1024 * 1024  # max_message_size
        self.offered_capabilities = frame[11] if len(frame) > 11 else None  # offered_capabilities
        self.remote_properties = frame[13] if len(frame) > 13 else None
